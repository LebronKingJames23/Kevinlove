...
INSERT INTO `t_customer` VALUES ('18', '觅心猎手', '法强', '217897987', '87742121', '不知道', '881112554', '1');
...
INSERT INTO `t_linkman` VALUES ('30', '智空', '男', '3123123', '65756756756', '6');
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `aa`(IN `username` varchar(20))
BEGIN
...
-- ----------------------------
-- Procedure structure for `addLinkMan`
-- ----------------------------
DROP PROCEDURE IF EXISTS `addLinkMan`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `addLinkMan`(IN `cid` varchar(20),IN `lkmName` varchar(20),IN `lkmGender` varchar(20),IN `lkmMobile` varchar(20),IN `lkmPhone` varchar(20))
BEGIN
	#Routine body goes here...
	insert into t_linkman(clid,lkmName,lkmGender,lkmMobile,lkmPhone) VALUES(cid,lkmName,lkmGender,lkmMobile,lkmPhone);
END
;;
DELIMITER ;

-- ----------------------------
-- Procedure structure for `deleteLinkman`
-- ----------------------------
DROP PROCEDURE IF EXISTS `deleteLinkman`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteLinkman`(IN `lid` integer)
BEGIN
	#Routine body goes here...
	delete from t_linkman  where linkid=lid;
END
;;
DELIMITER ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `findbyid`(IN `id` varchar(20))
BEGIN
	#Routine body goes here...
...
-- ----------------------------
-- Procedure structure for `queryCustomerByConditions`
-- ----------------------------
DROP PROCEDURE IF EXISTS `queryCustomerByConditions`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `queryCustomerByConditions`(IN `cname` varchar(20),IN `startRows` integer,IN `pageSize` integer)
BEGIN
	#Routine body goes here...
	select t.cid,t.custName,t.custSource,t.custPhone,t.custMobile,d.lname from t_customer t,t_dict d where t.custlevel=d.lid
            and custName like CONCAT('%',cname,'%')
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `selectUser`(IN `name` varchar(20),IN `pwd` varchar(50))
BEGIN
	#Routine body goes here...
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateCustomer`(IN `custName` varchar(20),IN `custSource` varchar(20),IN `custPhone` varchar(20),IN `custMobile` varchar(20),IN `custAddress` varchar(20),IN `custZip` varchar(20),IN `cid` integer)
BEGIN
	#Routine body goes here...
